...
# shift = ShiftStruct(mode='mean')
# item = img_iter(cam_path, '')
...
# while 1:
#     c = next(item)
#     roi  = cv2.selectROI('raw', c)
#     hsv = cv2.cvtColor(c, cv2.COLOR_RGB2HSV)
#     mask = np.zeros_like(c, shape=c.shape[:2])
#     grap_cutROI(c, roi, mask)
#     imshow('grap', mask)
#     colors = ShiftStruct.scan_inRange(hsv, mask)
#     print(colors)
#     if colors[0] is None or colors[1] is None: continue
#     shift.init_frame(c, roi, colors)
#     while cv2.waitKey(30) != ord(' '):
#         c = next(item)
#         shift.draw(c)
#         imshow('shift', c)
#         imshow('det', shift.src)
# exit()
size = (640, 480)
